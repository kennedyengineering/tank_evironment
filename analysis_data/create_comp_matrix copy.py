import glob
import os
import re
import ast
import pandas as pd
import matplotlib.pyplot as plt
import numpy as np

"""
Read as : agent1 vs agent2, agent1 wins % of the time against agent2.
"""


def parse_log_text(log_text):
    """
    Returns a dict with keys:
      - rewards: list of float
      - durations: list of int
      - win/tie/loss/timeout_log: list of int (0 or 1)
    """

    result = {}

    # Rewards list
    rewards_match = re.search(r"Rewards:\s*(\[[^\]]+\])", log_text)
    if rewards_match:
        result["rewards"] = ast.literal_eval(rewards_match.group(1))

    # Episode durations list
    durations_match = re.search(r"Episode Durations:\s*(\[[^\]]+\])", log_text)
    if durations_match:
        result["durations"] = ast.literal_eval(durations_match.group(1))

    # Extract binary logs: win, tie, loss, timeout
    for log_name in ["Win", "Tie", "Loss", "Timeout"]:
        pattern = rf"{log_name}\s+Log:\s*\[([^\]]+)\]"
        log_match = re.search(pattern, log_text, re.DOTALL)
        if log_match:
            # Find all 0 or 1 entries (with optional ".0" float format)
            entries = re.findall(r"\b[01](?:\.0*)?\b", log_match.group(1))
            result[f"{log_name.lower()}_log"] = [int(e) for e in entries]

    return result


if __name__ == "__main__":

    output_dir = "analysis_data/comp_matrix_diff/"
    os.makedirs(output_dir, exist_ok=True)

    paths = glob.glob("analysis_data/comp_package/*/logs/*.txt")

    records = []

    for path in paths:
        # --- derive agent names from the directory structure ---
        agent_pair = os.path.basename(os.path.dirname(os.path.dirname(path)))
        agent1_name, _, agent2_name = agent_pair.partition("_vs_")

        # --- derive map name and episode count from the filename ---
        map_episode_pair = os.path.splitext(os.path.basename(path))[0]
        map_name, episodes_str = map_episode_pair.rsplit("_", 1)
        episodes = int(episodes_str)

        # --- read & parse the log ---
        with open(path, "r") as f:
            text = f.read()
        info = parse_log_text(text)

        # --- validate naming convention ---
        assert episodes == len(info["win_log"])
        assert episodes == len(info["durations"])
        assert episodes == len(info["rewards"])

        # --- add to record ---
        records.append(
            {
                "map": map_name,
                "agent1": agent1_name,
                "agent2": agent2_name,
                "wins": info["win_log"].count(1),
                "ties": info["tie_log"].count(1),
                "losses": info["loss_log"].count(1),
                "timeouts": info["timeout_log"].count(1),
                "episodes": episodes,
            }
        )

    df = pd.DataFrame(records)

    df["win_rate"] = df["wins"] / df["episodes"]

    # agents = sorted(
    #     set(df['agent1']).union(df['agent2'])
    # )

    agents = [
        "model_train_v1_base",
        "model_train_v1",
        "model_finetuned_train_v0",
        "model_finetuned_train_v0_no_dense",
        "model_finetuned_train_v2",
        "model_finetuned_train_v2_no_dense",
    ]

    # aliases = {
    #     "model_train_v1_base": "non-LSTM",
    #     "model_train_v1": "LSTM",
    #     "model_finetuned_train_v0": "LSTM+self-dense",
    #     "model_finetuned_train_v0_no_dense": "LSTM+self-sparse",
    #     "model_finetuned_train_v2": "LSTM+ckpt-dense",
    #     "model_finetuned_train_v2_no_dense": "LSTM+ckpt-sparse",
    # }

    aliases = {
        "model_train_v1_base": "non-LSTM",
        "model_train_v1": "LSTM",
        "model_finetuned_train_v0": "LSTM+SD",
        "model_finetuned_train_v0_no_dense": "LSTM+SP",
        "model_finetuned_train_v2": "LSTM+CD",
        "model_finetuned_train_v2_no_dense": "LSTM+CS",
    }

    mats = {}

    for map_name in df["map"].unique():

        sub = df[df["map"] == map_name]

        # pivot to square matrix (agent1 down the rows, agent2 across the columns)
        mat = sub.pivot(index="agent1", columns="agent2", values="win_rate").reindex(
            index=agents, columns=agents, fill_value=np.nan
        )

        # pivot out losses, ties and episodes in the same layout
        loss_mat = sub.pivot(index="agent1", columns="agent2", values="losses").reindex(
            index=agents, columns=agents
        )
        ep_mat = sub.pivot(index="agent1", columns="agent2", values="episodes").reindex(
            index=agents, columns=agents
        )

        # compute the "reverse" win-rate for the lower half as losses / episodes
        reverse = loss_mat.T / ep_mat.T

        # fill only the strictly lower triangle of mat with reverse
        n = len(agents)
        for i in range(n):
            for j in range(n):
                if i > j and np.isnan(mat.iat[i, j]):
                    mat.iat[i, j] = reverse.iat[i, j]

        mats[map_name] = mat

        # draw heatmap
        fig, ax = plt.subplots(figsize=(6, 6))
        cax = ax.imshow(mat.values, vmin=0, vmax=1, aspect="equal")

        # annotate each cell with the win rate
        n = len(agents)
        for i in range(n):
            for j in range(n):
                rate = mat.iat[i, j]
                if not np.isnan(rate):
                    # choose a contrasting text color
                    color = "white" if rate < 0.5 else "black"
                    ax.text(
                        j,
                        i,
                        str(rate),
                        ha="center",
                        va="center",
                        color=color,
                        fontsize=8,
                    )

        # compute win-rate differential in percentage points
        diff_mat = (mat - mat.T) * 100

        # draw heatmap of differential
        fig, ax = plt.subplots(figsize=(6, 6))
        cax = ax.imshow(
            diff_mat.values,
            vmin=-100,
            vmax=100,
            # cmap="bwr",            # blue↔white↔red diverging
            aspect="equal",
        )

        # annotate each cell with the differential (signed integer)
        n = len(agents)
        for i in range(n):
            for j in range(n):
                d = diff_mat.iat[i, j]
                if not np.isnan(d):
                    txt = f"{d:+.1f}"  # e.g. "+20", "-35"
                    # use white text on strong colors, black otherwise
                    color = "white" if d < 0 else "black"
                    ax.text(
                        j, i, txt, ha="center", va="center", color=color, fontsize=10
                    )

        ax.set_xticks(np.arange(n))
        ax.set_yticks(np.arange(n))
        ax.set_xticklabels(
            [aliases[agent] for agent in agents], rotation=45, ha="right", fontsize=8
        )
        ax.set_yticklabels([aliases[agent] for agent in agents], fontsize=8)
        ax.set_xlabel("Opponent")
        ax.set_ylabel("Agent")

        map_name_title = ""
        for thing in map_name.split("_")[1:]:
            map_name_title += thing.capitalize() + " "
        map_name_title += f"({map_name.split('_')[0].capitalize()})"
        ax.set_title(f"Win-Rate Differential on Map {map_name_title}")

        # colorbar now in pp (percentage points)
        fig.colorbar(
            cax,
            ax=ax,
            orientation="vertical",
            fraction=0.04,
            pad=0.02,
            label="Win-Rate Δ (pp)",
        )
        plt.tight_layout(rect=[0, 0, 0.9, 1])
        plt.savefig(os.path.join(output_dir, f"comp_matrix_{map_name}_diff.png"))
        plt.savefig(os.path.join(output_dir, f"comp_matrix_{map_name}_diff.pdf"))

maps = sorted(df["map"].unique())

deterministic_maps = [m for m in maps if m.startswith("deterministic_")]
stochastic_maps = [m for m in maps if m.startswith("stochastic_")]

subset = deterministic_maps

dfs = []
for map in subset:
    dfs.append(mats[map])

df_avg = sum(dfs) / len(dfs)

# draw heatmap of differential
fig, ax = plt.subplots(figsize=(6, 6))
cax = ax.imshow(
    df_avg.values,
    vmin=0,
    vmax=1,
    # cmap="bwr",            # blue↔white↔red diverging
    aspect="equal",
)

# annotate each cell with the differential (signed integer)
n = len(agents)
for i in range(n):
    for j in range(n):
        d = df_avg.iat[i, j]
        if not np.isnan(d):
            txt = f"{d:.3f}"  # e.g. "+20", "-35"
            # use white text on strong colors, black otherwise
            color = "white" if d < 0.5 else "black"
            ax.text(j, i, txt, ha="center", va="center", color=color, fontsize=10)

ax.set_xticks(np.arange(n))
ax.set_yticks(np.arange(n))
ax.set_xticklabels(
    [aliases[agent] for agent in agents], rotation=45, ha="right", fontsize=8
)
ax.set_yticklabels([aliases[agent] for agent in agents], fontsize=8)
ax.set_xlabel("Opponent")
ax.set_ylabel("Agent")
ax.set_title(f"Mean Win-Rate on All Maps (Deterministic)")

# colorbar now in pp (percentage points)
fig.colorbar(
    cax, ax=ax, orientation="vertical", fraction=0.04, pad=0.02, label="Win Rate"
)
plt.tight_layout(rect=[0, 0, 0.9, 1])
plt.savefig(os.path.join(output_dir, f"comp_matrix_deterministic_all_average.png"))
plt.savefig(os.path.join(output_dir, f"comp_matrix_deterministic_all_average.pdf"))


# draw heatmap
fig, ax = plt.subplots(figsize=(6, 6))
cax = ax.imshow(mat.values, vmin=0, vmax=1, aspect="equal")

# annotate each cell with the win rate
n = len(agents)
for i in range(n):
    for j in range(n):
        rate = mat.iat[i, j]
        if not np.isnan(rate):
            # choose a contrasting text color
            color = "white" if rate < 0.5 else "black"
            ax.text(
                j,
                i,
                str(rate),
                ha="center",
                va="center",
                color=color,
                fontsize=7,
            )

# compute win-rate differential in percentage points
diff_mat = (df_avg - df_avg.T) * 100

# draw heatmap of differential
fig, ax = plt.subplots(figsize=(6, 6))
cax = ax.imshow(
    diff_mat.values,
    vmin=-100,
    vmax=100,
    # cmap="bwr",            # blue↔white↔red diverging
    aspect="equal",
)

# annotate each cell with the differential (signed integer)
n = len(agents)
for i in range(n):
    for j in range(n):
        d = diff_mat.iat[i, j]
        if not np.isnan(d):
            txt = f"{d:+.1f}"  # e.g. "+20", "-35"
            # use white text on strong colors, black otherwise
            color = "white" if d < 0 else "black"
            ax.text(j, i, txt, ha="center", va="center", color=color, fontsize=10)

ax.set_xticks(np.arange(n))
ax.set_yticks(np.arange(n))
ax.set_xticklabels(
    [aliases[agent] for agent in agents], rotation=45, ha="right", fontsize=8
)
ax.set_yticklabels([aliases[agent] for agent in agents], fontsize=8)
ax.set_xlabel("Opponent")
ax.set_ylabel("Agent")
ax.set_title(f"Win-Rate Differential on All Maps (Deterministic)")

# colorbar now in pp (percentage points)
fig.colorbar(
    cax, ax=ax, orientation="vertical", fraction=0.04, pad=0.02, label="Win-Rate Δ (pp)"
)
plt.tight_layout(rect=[0, 0, 0.9, 1])
plt.savefig(os.path.join(output_dir, f"comp_matrix_deterministic_all_average_diff.png"))
plt.savefig(os.path.join(output_dir, f"comp_matrix_deterministic_all_average_diff.pdf"))


subset = stochastic_maps

dfs = []
for map in subset:
    dfs.append(mats[map])

df_avg = sum(dfs) / len(dfs)

# draw heatmap of differential
fig, ax = plt.subplots(figsize=(6, 6))
cax = ax.imshow(
    df_avg.values,
    vmin=0,
    vmax=1,
    # cmap="bwr",            # blue↔white↔red diverging
    aspect="equal",
)

# annotate each cell with the differential (signed integer)
n = len(agents)
for i in range(n):
    for j in range(n):
        d = df_avg.iat[i, j]
        if not np.isnan(d):
            txt = f"{d:.3f}"  # e.g. "+20", "-35"
            # use white text on strong colors, black otherwise
            color = "white" if d < 0.5 else "black"
            ax.text(j, i, txt, ha="center", va="center", color=color, fontsize=10)

ax.set_xticks(np.arange(n))
ax.set_yticks(np.arange(n))
ax.set_xticklabels(
    [aliases[agent] for agent in agents], rotation=45, ha="right", fontsize=8
)
ax.set_yticklabels([aliases[agent] for agent in agents], fontsize=8)
ax.set_xlabel("Opponent")
ax.set_ylabel("Agent")
ax.set_title(f"Mean Win-Rate on All Maps (Stochastic)")

# colorbar now in pp (percentage points)
fig.colorbar(
    cax, ax=ax, orientation="vertical", fraction=0.04, pad=0.02, label="Win Rate"
)
plt.tight_layout(rect=[0, 0, 0.9, 1])
plt.savefig(os.path.join(output_dir, f"comp_matrix_stochastic_all_average.png"))
plt.savefig(os.path.join(output_dir, f"comp_matrix_stochastic_all_average.pdf"))


# draw heatmap
fig, ax = plt.subplots(figsize=(6, 6))
cax = ax.imshow(mat.values, vmin=0, vmax=1, aspect="equal")

# annotate each cell with the win rate
n = len(agents)
for i in range(n):
    for j in range(n):
        rate = mat.iat[i, j]
        if not np.isnan(rate):
            # choose a contrasting text color
            color = "white" if rate < 0.5 else "black"
            ax.text(
                j,
                i,
                str(rate),
                ha="center",
                va="center",
                color=color,
                fontsize=7,
            )

# compute win-rate differential in percentage points
diff_mat = (df_avg - df_avg.T) * 100

# draw heatmap of differential
fig, ax = plt.subplots(figsize=(6, 6))
cax = ax.imshow(
    diff_mat.values,
    vmin=-100,
    vmax=100,
    # cmap="bwr",            # blue↔white↔red diverging
    aspect="equal",
)

# annotate each cell with the differential (signed integer)
n = len(agents)
for i in range(n):
    for j in range(n):
        d = diff_mat.iat[i, j]
        if not np.isnan(d):
            txt = f"{d:+.1f}"  # e.g. "+20", "-35"
            # use white text on strong colors, black otherwise
            color = "white" if d < 0 else "black"
            ax.text(j, i, txt, ha="center", va="center", color=color, fontsize=10)

ax.set_xticks(np.arange(n))
ax.set_yticks(np.arange(n))
ax.set_xticklabels(
    [aliases[agent] for agent in agents], rotation=45, ha="right", fontsize=8
)
ax.set_yticklabels([aliases[agent] for agent in agents], fontsize=8)
ax.set_xlabel("Opponent")
ax.set_ylabel("Agent")
ax.set_title(f"Win-Rate Differential on All Maps (Stochastic)")

# colorbar now in pp (percentage points)
fig.colorbar(
    cax, ax=ax, orientation="vertical", fraction=0.04, pad=0.02, label="Win-Rate Δ (pp)"
)
plt.tight_layout(rect=[0, 0, 0.9, 1])
plt.savefig(os.path.join(output_dir, f"comp_matrix_stochastic_all_average_diff.png"))
plt.savefig(os.path.join(output_dir, f"comp_matrix_stochastic_all_average_diff.pdf"))


subset = [m for m in stochastic_maps if m not in "stochastic_random"]
print(subset)

dfs = []
for map in subset:
    dfs.append(mats[map])

df_avg = sum(dfs) / len(dfs)

# draw heatmap of differential
fig, ax = plt.subplots(figsize=(6, 6))
cax = ax.imshow(
    df_avg.values,
    vmin=0,
    vmax=1,
    # cmap="bwr",            # blue↔white↔red diverging
    aspect="equal",
)

# annotate each cell with the differential (signed integer)
n = len(agents)
for i in range(n):
    for j in range(n):
        d = df_avg.iat[i, j]
        if not np.isnan(d):
            txt = f"{d:.3f}"  # e.g. "+20", "-35"
            # use white text on strong colors, black otherwise
            color = "white" if d < 0.5 else "black"
            ax.text(j, i, txt, ha="center", va="center", color=color, fontsize=10)

ax.set_xticks(np.arange(n))
ax.set_yticks(np.arange(n))
ax.set_xticklabels(
    [aliases[agent] for agent in agents], rotation=45, ha="right", fontsize=8
)
ax.set_yticklabels([aliases[agent] for agent in agents], fontsize=8)
ax.set_xlabel("Opponent")
ax.set_ylabel("Agent")
ax.set_title(f"Mean Win-Rate on Maze-Like Maps (Stochastic)")

# colorbar now in pp (percentage points)
fig.colorbar(
    cax, ax=ax, orientation="vertical", fraction=0.04, pad=0.02, label="Win Rate"
)
plt.tight_layout(rect=[0, 0, 0.9, 1])
plt.savefig(os.path.join(output_dir, f"comp_matrix_stochastic_maze_average.png"))
plt.savefig(os.path.join(output_dir, f"comp_matrix_stochastic_maze_average.pdf"))


# draw heatmap
fig, ax = plt.subplots(figsize=(6, 6))
cax = ax.imshow(mat.values, vmin=0, vmax=1, aspect="equal")

# annotate each cell with the win rate
n = len(agents)
for i in range(n):
    for j in range(n):
        rate = mat.iat[i, j]
        if not np.isnan(rate):
            # choose a contrasting text color
            color = "white" if rate < 0.5 else "black"
            ax.text(
                j,
                i,
                str(rate),
                ha="center",
                va="center",
                color=color,
                fontsize=7,
            )

# compute win-rate differential in percentage points
diff_mat = (df_avg - df_avg.T) * 100

# draw heatmap of differential
fig, ax = plt.subplots(figsize=(6, 6))
cax = ax.imshow(
    diff_mat.values,
    vmin=-100,
    vmax=100,
    # cmap="bwr",            # blue↔white↔red diverging
    aspect="equal",
)

# annotate each cell with the differential (signed integer)
n = len(agents)
for i in range(n):
    for j in range(n):
        d = diff_mat.iat[i, j]
        if not np.isnan(d):
            txt = f"{d:+.1f}"  # e.g. "+20", "-35"
            # use white text on strong colors, black otherwise
            color = "white" if d < 0 else "black"
            ax.text(j, i, txt, ha="center", va="center", color=color, fontsize=10)

ax.set_xticks(np.arange(n))
ax.set_yticks(np.arange(n))
ax.set_xticklabels(
    [aliases[agent] for agent in agents], rotation=45, ha="right", fontsize=8
)
ax.set_yticklabels([aliases[agent] for agent in agents], fontsize=8)
ax.set_xlabel("Opponent")
ax.set_ylabel("Agent")
ax.set_title(f"Win-Rate Differential on Maze-Like Maps (Stochastic)")

# colorbar now in pp (percentage points)
fig.colorbar(
    cax, ax=ax, orientation="vertical", fraction=0.04, pad=0.02, label="Win-Rate Δ (pp)"
)
plt.tight_layout(rect=[0, 0, 0.9, 1])
plt.savefig(os.path.join(output_dir, f"comp_matrix_stochastic_maze_average_diff.png"))
plt.savefig(os.path.join(output_dir, f"comp_matrix_stochastic_maze_average_diff.pdf"))


subset = [m for m in deterministic_maps if m not in "deterministic_random"]
print(subset)

dfs = []
for map in subset:
    dfs.append(mats[map])

df_avg = sum(dfs) / len(dfs)

# draw heatmap of differential
fig, ax = plt.subplots(figsize=(6, 6))
cax = ax.imshow(
    df_avg.values,
    vmin=0,
    vmax=1,
    # cmap="bwr",            # blue↔white↔red diverging
    aspect="equal",
)

# annotate each cell with the differential (signed integer)
n = len(agents)
for i in range(n):
    for j in range(n):
        d = df_avg.iat[i, j]
        if not np.isnan(d):
            txt = f"{d:.3f}"  # e.g. "+20", "-35"
            # use white text on strong colors, black otherwise
            color = "white" if d < 0.5 else "black"
            ax.text(j, i, txt, ha="center", va="center", color=color, fontsize=10)

ax.set_xticks(np.arange(n))
ax.set_yticks(np.arange(n))
ax.set_xticklabels(
    [aliases[agent] for agent in agents], rotation=45, ha="right", fontsize=8
)
ax.set_yticklabels([aliases[agent] for agent in agents], fontsize=8)
ax.set_xlabel("Opponent")
ax.set_ylabel("Agent")
ax.set_title(f"Mean Win-Rate on Maze-Like Maps (Deterministic)")

# colorbar now in pp (percentage points)
fig.colorbar(
    cax, ax=ax, orientation="vertical", fraction=0.04, pad=0.02, label="Win Rate"
)
plt.tight_layout(rect=[0, 0, 0.9, 1])
plt.savefig(os.path.join(output_dir, f"comp_matrix_deterministic_maze_average.png"))
plt.savefig(os.path.join(output_dir, f"comp_matrix_deterministic_maze_average.pdf"))


# draw heatmap
fig, ax = plt.subplots(figsize=(6, 6))
cax = ax.imshow(mat.values, vmin=0, vmax=1, aspect="equal")

# annotate each cell with the win rate
n = len(agents)
for i in range(n):
    for j in range(n):
        rate = mat.iat[i, j]
        if not np.isnan(rate):
            # choose a contrasting text color
            color = "white" if rate < 0.5 else "black"
            ax.text(
                j,
                i,
                str(rate),
                ha="center",
                va="center",
                color=color,
                fontsize=7,
            )

# compute win-rate differential in percentage points
diff_mat = (df_avg - df_avg.T) * 100

# draw heatmap of differential
fig, ax = plt.subplots(figsize=(6, 6))
cax = ax.imshow(
    diff_mat.values,
    vmin=-100,
    vmax=100,
    # cmap="bwr",            # blue↔white↔red diverging
    aspect="equal",
)

# annotate each cell with the differential (signed integer)
n = len(agents)
for i in range(n):
    for j in range(n):
        d = diff_mat.iat[i, j]
        if not np.isnan(d):
            txt = f"{d:+.1f}"  # e.g. "+20", "-35"
            # use white text on strong colors, black otherwise
            color = "white" if d < 0 else "black"
            ax.text(j, i, txt, ha="center", va="center", color=color, fontsize=10)

ax.set_xticks(np.arange(n))
ax.set_yticks(np.arange(n))
ax.set_xticklabels(
    [aliases[agent] for agent in agents], rotation=45, ha="right", fontsize=8
)
ax.set_yticklabels([aliases[agent] for agent in agents], fontsize=8)
ax.set_xlabel("Opponent")
ax.set_ylabel("Agent")
ax.set_title(f"Win-Rate Differential on Maze-Like Maps (Deterministic)")

# colorbar now in pp (percentage points)
fig.colorbar(
    cax, ax=ax, orientation="vertical", fraction=0.04, pad=0.02, label="Win-Rate Δ (pp)"
)
plt.tight_layout(rect=[0, 0, 0.9, 1])
plt.savefig(
    os.path.join(output_dir, f"comp_matrix_deterministic_maze_average_diff.png")
)
plt.savefig(
    os.path.join(output_dir, f"comp_matrix_deterministic_maze_average_diff.pdf")
)
