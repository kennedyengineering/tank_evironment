...
class RunFarRTL(MapData):
    ...
    tank_map_data: list[TankMapData] = field(
        default_factory=lambda: [
            ...
            TankMapData(position_x=85, position_y=35, angle=np.pi / 2),
        ]
    )
...
class RunFarLTR(MapData):
    ...
    tank_map_data: list[TankMapData] = field(
        default_factory=lambda: [
            TankMapData(position_x=50, position_y=85, angle=-np.pi),
            TankMapData(position_x=15, position_y=35, angle=-np.pi / 2),
        ]
    )
