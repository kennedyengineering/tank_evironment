...
def plot_conv(data: NDArray[np.float64], save_path=None) -> None:
    """
    ...
    for f_idx in range(n_filters):
        ...
        # Show heatmap
        mappable = ax.imshow(plot_data, cmap="viridis", origin="lower", aspect="auto")
...
def plot_latent(data: NDArray[np.float64], save_path=None) -> None:
...
def plot_latent_heatmap(data: NDArray[np.float64], save_path=None) -> None:
